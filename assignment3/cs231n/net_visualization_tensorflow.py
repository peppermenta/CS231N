...
def compute_saliency_maps(X, y, model):
    ...
    with tf.GradientTape() as tape:
        scores = model.call(tfX)
        correct_class_scores = tf.gather_nd(scores,tf.stack((tf.range(N),y),axis=1))
        
        gradients = tape.gradient(correct_class_scores,tfX)
...
def make_fooling_image(X, target_y, model):
    ...
    for i in range(100):
        with tf.GradientTape() as tape:
            ...
            desired_score = scores[:,target_y]

            grad = tape.gradient(desired_score,tfX)

            grad /= np.sqrt(np.sum(grad**2))

            X_fooling += learning_rate*grad
...
def class_visualization_update_step(X, model, target_y, l2_reg, learning_rate):
    ...
    with tf.GradientTape() as tape:
        ...
        grad = tape.gradient(target_score,tfX)
